...
namespace CharacterController.Collections
{
    ...
    public unsafe class SoACharaDataDic : IDisposable
    {
        ...
        public SoACharaDataDic(int maxCapacity = DEFAULT_MAX_CAPACITY, Allocator allocator = Allocator.Persistent)
        {
            ...
            this._coldLog = new UnsafeList<CharacterColdLog>(
                (CharacterColdLog*)(this._bulkMemory + layout.ColdLogOffset),
                maxCapacity
            );
        ...
        public int Add(GameObject obj, CharacterBaseInfo baseInfo, CharacterAtkStatus atkStatus,
                      ...
                      MoveStatus moveStatus, CharacterColdLog coldLog, BaseController controller)
        {
            if ( obj == null )
            {
        ...
        public int Add(BrainStatus status, BaseController controller, int hashCode)
        {
            ...
            return this.AddByHash(hashCode, baseInfo, atkStatus, defStatus, solidData,
                           stateInfo, moveStatus, coldLog, controller);
        }
        ...
        public bool RemoveByHash(int hashCode)
        {
            ...
            {
                // ハッシュ保存
                int movedHash = this._coldLog[lastIndex].hashCode;

                // 最後の要素と削除対象要素を入れ替える削除（メモリ再割り当てなし）
                this._characterBaseInfo.RemoveAtSwapBack(dataIndex);
                this._characterAtkStatus.RemoveAtSwapBack(dataIndex);
                this._characterDefStatus.RemoveAtSwapBack(dataIndex);
                this._solidData.RemoveAtSwapBack(dataIndex);
                this._characterStateInfo.RemoveAtSwapBack(dataIndex);
                this._moveStatus.RemoveAtSwapBack(dataIndex);
                this._coldLog.RemoveAtSwapBack(dataIndex);

                // マネージド配列は手動で最後の要素と入れ替え
                ...
                // 参照をクリア
                this._controllers[lastIndex] = null;
            }
            else
            {
                // リストの長さを減らす
        ...
        public bool TryGetValue(GameObject obj, out CharacterBaseInfo baseInfo, out CharacterAtkStatus atkStatus,
                               ...
                               out MoveStatus moveStatus, out CharacterColdLog coldLog,
        ...
        public bool TryGetByIndex(int index, out CharacterBaseInfo baseInfo, out CharacterAtkStatus atkStatus,
                                 ...
                                 out MoveStatus moveStatus, out CharacterColdLog coldLog, out BaseController controller)
        {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ref CharacterColdLog GetCharaColdLogByIndex(int index)
        {
            if ( index < 0 || index >= this._count )
            {
                throw new ArgumentOutOfRangeException(nameof(index));
        ...
        public void ForEach(Action<int, CharacterBaseInfo, CharacterAtkStatus, CharacterDefStatus, SolidData,
                                  CharacterStateInfo, MoveStatus, CharacterColdLog, BaseController> action)
        {
            if ( action == null )
            {
        ...
        private MemoryLayout CalculateMemoryLayout(int capacity)
        {
            ...
            currentOffset += capacity * sizeof(CharacterColdLog);
